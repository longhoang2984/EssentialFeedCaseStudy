...
//
//  Created by Cửu Long Hoàng on 08/10/2022.
//
...
class EssentialFeedCacheIntegrationTests: XCTestCase {
    ...
    func test_saveFeed_overridesItemsSavedOnASeparateInstance() {
        ...
        let feadLoaderToPerformLoad = makeFeedLoader()
        let firstFeed = uniqueFeeds().models
        let latestFeed = uniqueFeeds().models
        
        save(firstFeed, with: feedLoaderToPerformFirstSave)
        save(latestFeed, with: feedLoaderToPerformLastSave)
        
        expect(feadLoaderToPerformLoad, toLoad: latestFeed)
    }
    ...
    func test_saveImageData_overridesSavedImageDataOnASeparateInstance() {
        ...
        let feedLoader = makeFeedLoader()
        let image = uniqueFeed()
        let fistImageData = Data("first".utf8)
        let lastImageData = Data("last".utf8)
        
        save([image], with: feedLoader)
        save(fistImageData, for: image.url, with: imageLoaderToPerformFirstSave)
    ...
    private func validateCache(with feedLoader: LocalFeedLoader, file: StaticString = #file, line: UInt = #line) {
        ...
        feedLoader.validateCache() { result in
            if case let Result.failure(error) = result {
                XCTAssertNil(error, "Expected to validate feed successfully, got error: \(error) instead", file: file, line: line)
            }
            saveExp.fulfill()
        }
    ...
    private func save(_ feed: [FeedImage], with loader: LocalFeedLoader, file: StaticString = #file, line: UInt = #line) {
        ...
        loader.save(feed) { result in
            if case let Result.failure(error) = result {
                XCTAssertNil(error, "Expected to save feed successfully, got error: \(error) instead", file: file, line: line)
            }
            saveExp.fulfill()
        }
        wait(for: [saveExp], timeout: 1.0)
    ...
    private func expect(_ sut: LocalFeedImageDataLoader, toLoad expectedData: Data, for url: URL, file: StaticString = #file, line: UInt = #line) {
        ...
        _ = sut.loadImageData(from: url, completion: { result in
            ...
            case let .failure(loadedError):
                XCTFail("Expected successful image data result, got error: \(loadedError) instead", file: file, line: line)
            }
            exp.fulfill()
        })
        wait(for: [exp], timeout: 1.0)
